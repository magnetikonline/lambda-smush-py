#!/usr/bin/env python
"""
Utility with the sole aim to squeeze that little bit more code out of Python based
AWS Lambda functions defined in-line via CloudFormation templates.
"""
import argparse
import base64
import os.path
...
def read_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Generates compressed Python based AWS Lambda functions to fit within the '
    ...
    def get_file_content(file_path):
        """Get File Content from 'file_path'"""
        fh = open(file_path, 'r')
        content = fh.read()
        fh.close()
        return content
    ...
    if (arg_list.template is not None):
        if (not os.path.isfile(arg_list.template)):
            exit_error('unable to locate CloudFormation YAML template [{0}]'.format(
                arg_list.template))
        if (
            ...
            exit_error(
                'argument --template-placeholder must be supplied with --template')
        ...
        if (template_placeholder is not None):
            exit_error(
                'argument --template-placeholder only works with --template')
    # return arguments
    return (
        lambda_source,
        handler_name,
...
def get_source_compressed(lambda_source, strip_comments, strip_empty_lines):
...
def build_bootloader(lambda_handler_name, source_compressed):
    """Oh hai, look at me a fancy bootloader"""
...
def {1}(e,c):
    return m.{1}(e,c)
'''.format(build_base64_source(), lambda_handler_name)
...
def main():
    ...
    if (output_filename):
        ...
        fh.write(result)
        fh.close()
        print('Generated Lambda function written to [{0}]'.format(
            output_filename))
    else:
        print(result)
